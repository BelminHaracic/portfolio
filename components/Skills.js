...
export default function Skills() {
  // State za praćenje veličine ekrana
  const [isMobile, setIsMobile] = useState(false);
  
  // Hook za detekciju veličine ekrana
  useEffect(() => {
      ...
      <div className={styles.skillsGrid}>
        {skills.map((skill, index) => (
          <div key={index} className={styles.skillCard}>
            ...
            {/* Prikazujemo opis direktno za mobilne uređaje umesto tooltipa */}
            {isMobile && <span className={styles.mobileDescription}>{skill.description}</span>}
          </div>
        ))}
      </div>
    </section>
